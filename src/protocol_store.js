...
class Storage {
  ...
  _getAll(namespace) {
    const collection = [];
    for (let id of Object.keys(this._store.data._private)) {
  ...
  _getAllIds(namespace) {
    const collection = [];
    for (let key of Object.keys(this._store.data._private)) {
  ...
  _remove(namespace, id) {
    this._store.remove("" + namespace + id);
  }
